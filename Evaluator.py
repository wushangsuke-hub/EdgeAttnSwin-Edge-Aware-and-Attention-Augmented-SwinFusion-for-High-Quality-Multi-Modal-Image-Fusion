...
class Evaluator():
    ...
    def SSIM(cls, image_F, image_A, image_B):
        cls.input_check(image_F, image_A, image_B)
        # Use local ssim_index implementation for robust handling (returns mssim, map, ...)
        try:
            mssim1, _, _, _ = cls.ssim_index(image_F, image_A)
            mssim2, _, _, _ = cls.ssim_index(image_F, image_B)
            return mssim1 + mssim2
        except Exception:
            # fallback to skimage's ssim with explicit data_range
            try:
                return ssim(image_F, image_A, data_range=np.max(image_F) - np.min(image_F)) + \
                       ssim(image_F, image_B, data_range=np.max(image_F) - np.min(image_F))
            except Exception:
                return float('nan')

    @classmethod
    def evaluate(cls, image_F, image_A, image_B):
        """Compute evaluation metrics used in the paper's Table 4.

        Returns a dict with keys: 'MI', 'SCD', 'VIF', 'Qabf', 'SSIM'.
        Images are expected as 2D numpy arrays (grayscale / Y channel).
        """
        cls.input_check(image_F, image_A, image_B)
        results = {}
        try:
            results['MI'] = float(cls.MI(image_F, image_A, image_B))
        except Exception:
            results['MI'] = np.nan
        try:
            results['SCD'] = float(cls.SCD(image_F, image_A, image_B))
        except Exception:
            results['SCD'] = np.nan
        try:
            # 关键修正：调用独立的VIFF函数，确保与原论文VIF计算逻辑一致
            results['VIF'] = float(VIFF(image_F, image_A, image_B))
        except Exception:
            results['VIF'] = np.nan
        try:
            results['Qabf'] = float(cls.Qabf(image_F, image_A, image_B))
        except Exception:
            results['Qabf'] = np.nan
        try:
            results['SSIM'] = float(cls.SSIM(image_F, image_A, image_B))
        except Exception:
            results['SSIM'] = np.nan
        return results
